...
const handleSubmitStatus = async () => {
 ...
 const filteredCategories = useMemo(() => {
  ...
  return categories.filter((cat) => {
    return (
      String(cat.subCategoryId ?? "").toLowerCase().includes(term) ||
      String(cat.categoryName ?? "").toLowerCase().includes(term) ||
      String(cat.subCategory ?? "").toLowerCase().includes(term) ||
      ...
      <div className="category-table pb-3">
        ...
        >
          ...
          <div className="input-group align-items-center mx-3 py-2" style={{ maxWidth: '350px', width: '100%' }}>
            ...
            />
            <button type="button" className="btn btn-light btn-sm ms-0" style={{ height: '34px', width: '34px' }}>
